...
public class GameUI extends Thread
{
	...
	public void DrawSelect() 
	{
		...
		while(States == 1) 
		{
			...
			SelectionHover.fillRect(selectionkeyboard.x, selectionkeyboard.y, SelectionMenuInput.Easy.w, SelectionMenuInput.Easy.h);
			Selections.drawString("EASY", SelectionMenuInput.Easy.x+65,SelectionMenuInput.Easy.y+62);
			Selections.drawString("MEDIUM",SelectionMenuInput.Medium.x+25,SelectionMenuInput.Medium.y+62);
			Selections.drawString("HARD",SelectionMenuInput.Hard.x+60,SelectionMenuInput.Hard.y+62);
			Selections.drawString("CUSTOM",SelectionMenuInput.Custom.x+25,SelectionMenuInput.Custom.y+62);
			...
			Selections.drawRect(SelectionMenuInput.Custom.x,SelectionMenuInput.Custom.y,SelectionMenuInput.Custom.w,SelectionMenuInput.Custom.h);
			try {
				Thread.sleep(12);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
	...
	{	
		...
		while(States == 2) 
		{
			...
			if(!new Generate().toString(game).contains("0") && !keyboard.Conflict && Loading == false) 
			{
				keyboard.Hover = 0;
				Finish = true;
			}
			...
			for(int i = 0; i < 9; ++i) 
			{
				for(int j =  0; j <  9; ++j) 
				{
					...
					if(Loading == true) 
					{
						ConFlict.drawString("LOADING..."+loadingvar, keyboard.BigBox.x+125,keyboard.BigBox.y-20);
					}
					else 
					{
						if(game[x][y] != 0) 
			...
			if(Loading == false) 
			{
				...
				{
					puzzlebox.drawString("DIFFICULTY: CUSTOM", keyboard.BigBox.x+5, keyboard.savegame.y+30);
				}
				...
				if(keyboard.solvehover == 1) 
				{
					Hoverframe.fillRect(keyboard.solvecoodx, keyboard.solvecoody, keyboard.Solve.w, keyboard.Solve.h);
				}
				...
				OutLine.drawRect(keyboard.Solve.x, keyboard.Solve.y, keyboard.Solve.w, keyboard.Solve.h);
				puzzlebox.drawString("SAVE GAME", keyboard.savegame.x+15,keyboard.savegame.y+30);
				puzzlebox.drawString("SOLVE",keyboard.Solve.x+43, keyboard.Solve.y+30);
			}
			if(Finish == true && !Difficulty.equals("C")) 
			{
				ConFlict.drawString("COMPLETE!", keyboard.BigBox.x,keyboard.nextgame.y+40);
				if(keyboard.nextgamehover == 1) 
				{
					Hoverframe.fillRect(keyboard.nextgamecoodx, keyboard.nextgamecoody,keyboard.nextgame.w,keyboard.nextgame.h);
				}
				OutLine.drawRect(keyboard.nextgame.x,keyboard.nextgame.y,keyboard.nextgame.w,keyboard.nextgame.h);
				puzzlebox.drawString("NEXT GAME",keyboard.nextgame.x+15,keyboard.nextgame.y+30);
	...
	public void run()
	{
		 while(true) 
		 {
			 while(Difficulty == "" || Difficulty.equals("C")) 
				{
				 try {
						Thread.sleep(16);
					} catch (InterruptedException e) {
	...
	public void StartGame(GameUI game)
	{
		while(true) 
		{
			...
			if(!gamesound.background.isActive());
				gamesound.Background();
			if(!game.isAlive()) 
				game.start();
			game.DrawMain();
			game.DrawSelect();
